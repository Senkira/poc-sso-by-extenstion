...
if ($manifest.version -ne '0.12.3') { throw 'Unexpected extension version.' }
...
if ($html -notmatch '/app.js\?v=0\.12\.3' -or $html -notmatch '/styles.css\?v=0\.12\.3') { throw 'Hosted assets are not cache-busted.' }
...
if ($worker -notmatch 'OPENING_ISOLATED_GEMINI_TAB' -or $worker -notmatch 'tabs\.reload' -or $worker -notmatch 'tabs\.remove') { throw 'Isolated Gemini tab/reload/single-tab handoff is missing.' }
