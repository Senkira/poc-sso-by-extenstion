...
async function main() {
  ...
  assert.match(elements.get("#connection-detail").textContent, /v0\.3\.0.*v0\.4\.0.*Reload/);

  pingProtocol = 2;
  await elements.get("#retry-button").listeners.click();
  await flush();
  assert.match(elements.get("#connection-detail").textContent, /protocol เก่า.*Reload.*v0\.4\.0/);
